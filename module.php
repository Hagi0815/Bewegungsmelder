...
class Rolladensteuerung extends IPSModuleStrict
{
    ...
    public function ControlBlind(bool $considerDeactivationTimeAuto): bool
    {
        ...
        if ($dayStateChanged) {
            ...
            $blockReason = $this->shouldBlockMovement(
        ...
        $reasonText = $Hinweis !== '' ? $Hinweis : ($bNoMove ? 'Sperre (Grund unbekannt)' : 'Keine Änderung nötig');
    ...
    {
        if ($bNoMove) {
            // hint bleibt leer; der Sperrgrund wurde bereits in ControlBlind als $Hinweis gesetzt
     ...
     */
    /**
     * Prüft ob die automatische Bewegung gesperrt werden soll.
     *
     * @return string Leerstring = keine Sperre; sonst menschenlesbarer Sperrgrund (→ bNoMove=true)
     */
    private function shouldBlockMovement(
        ...
        // 1. Karenzzeit nach automatischer Bewegung: letzte Bewegung war durch Automatik → kein Block
        if ($tsBlindLastMovement <= strtotime('+5 sec', $tsAutomatik)) {
            return '';
        }

        // 2. Manuellen Status synchronisieren
        ...
        if ($tsManual <= $tsIsDayChanged) {
            // Manuelle Bewegung war vor dem letzten Tag/Nacht-Wechsel → Sperre nicht mehr gültig
            $this->Logger_Dbg(__FUNCTION__, sprintf(
                'Manuelle Bewegung (%s) vor letztem Tag/Nacht-Wechsel (%s) → keine Sperre',
                date('H:i:s', $tsManual),
                date('H:i:s', $tsIsDayChanged)
            ));
            return '';
        }
        ...
        // 3. Sperr-Logik
        if (!$isDay) {
            $reason = sprintf('Manuelle Bewegung um %s (Nacht → Sperre dauerhaft)', date('H:i:s', $tsManual));
            $this->Logger_Dbg(__FUNCTION__, $reason);
            return $reason;
        }

        // Tagsüber: Sperre wenn geschlossen oder noch innerhalb der Deaktivierungszeit
        $deactivationTimeManuSecs = $this->ReadPropertyInteger(self::PROP_DEACTIVATIONMANUALMOVEMENT) * 60;
        $deactivationMinutes      = (int)($deactivationTimeManuSecs / 60);

        $isClosed = ($blindLevelAct === $this->profileBlindLevel['MaxValue']) &&
                    ($slatsLevelAct === ($this->profileSlatsLevel['MaxValue'] ?? null));

        if ($isClosed) {
            $reason = sprintf('Manuell geschlossen um %s (Rollladen zu → Sperre bis Tageswechsel)', date('H:i:s', $tsManual));
            $this->Logger_Dbg(__FUNCTION__, $reason);
            return $reason;
        }

        if ($deactivationTimeManuSecs === 0) {
            $reason = sprintf('Manuelle Bewegung um %s (Sperrzeit = unbegrenzt)', date('H:i:s', $tsManual));
            $this->Logger_Dbg(__FUNCTION__, $reason);
            return $reason;
        }

        $tsExpiry = strtotime("+ $deactivationTimeManuSecs seconds", $tsManual);
        if ($tsExpiry > time()) {
            $reason = sprintf(
                'Manuelle Bewegung um %s (Sperre %d Min, läuft ab um %s)',
                date('H:i:s', $tsManual),
                $deactivationMinutes,
                date('H:i:s', $tsExpiry)
            );
        ...
        $this->Logger_Dbg(__FUNCTION__, sprintf(
            'Sperrzeit (%d Min nach manueller Bewegung um %s) abgelaufen → Automatik freigegeben',
            $deactivationMinutes,
            date('H:i:s', $tsManual)
        ));
        $this->resetManualMovement();
